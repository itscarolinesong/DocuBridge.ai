...
//   ],
//     pattern: /Patent ID:\s*([A-Za-z0-9-]+)/i
//   },
//   dateOfBirth: {
//     variants: [
...
//   const patientId = extractField(cleanedText, fieldVariants.patientId.variants, fieldVariants.patientId.pattern) || 'N/A';
//   const dateOfBirth = extractField(cleanedText, fieldVariants.dateOfBirth.variants, fieldVariants.dateOfBirth.pattern) || 'N/A';
//   const diagnosis = extractField(cleanedText, fieldVariants.diagnosis.variants, fieldVariants.diagnosis.pattern) || 'N/A';
...
//   // Look for CLINICAL DATA section specifically
//   const clinicalDataMatch = text.match(/CLINICAL DATA:\s*([R\d\s;.]+)/i);
//   if (clinicalDataMatch) {
//     const [, data] = clinicalDataMatch;
//     // Split by semicolon and clean up each code
//     return data.split(';').map(code => code.trim()).filter(code => code.length > 0);
//   }

//   // Fallback to original method
//   const labHeaders = ['lab results', 'labs', 'laboratory', 'lab values', 'test results', 'laboratory results', 'blood work', 'clinical data'];
...
const fieldVariants = {
  patientName: {
    variants: [
      'patient name', 'name', 'full name', 'pt name', 'patient', 
      'name of patient', 'patient full name', 'legal name', 'patent test'
    ],
    pattern: /Patent\s+(.+?)\s+Age:/i
  },
  patientId: {
    variants: [
      'mrn', 'patient id', 'id', 'medical record number', 'record number',
...
function extractLabResults(text: string): string[] {
  // Look for CLINICAL DATA section specifically
  const clinicalDataMatch = text.match(/CLINICAL DATA:\s*([^:\n]+)/i);
  if (clinicalDataMatch) {
    const [, data] = clinicalDataMatch;
    // Split by semicolon and clean up each code
    const codes = data.split(';').map(code => code.trim()).filter(code => code.length > 0);
    if (codes.length > 0) {
      return codes;
    }
  }
